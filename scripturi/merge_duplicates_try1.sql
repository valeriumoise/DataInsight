CREATE OR REPLACE FUNCTION merge_duplicates (
    ...
    cursor_dups sys_refcursor;
    id_cursor sys_refcursor;
    dups_wo_ig sys_refcursor;
BEGIN
...
--    open cursor_dups for 'select * from ' || in_table_name || ' where rowid not in (select max(rowid) from ' || in_table_name || ' group by group_by_str);';
--    
--    for v_dup in cursor_dups loop
--        open dups_wo_ig for 'select ' || group_by_str || ' from in ' || in_table_name || ' where rowid not in (select max(rowid) from in_table_name group by group_by_str) and ' || table_pk || '<=' || v_dup.pk;
--        open id_cursor for 'select ' || table_pk || ' as pk from in ' || in_table_name || ' where rowid not in (select min(rowid) from in_table_name group by group_by_str) and (' || group_by_str || ') = ( ' || dups_wo_ig || ' );';
